...
void test5 ()
{
    ...
    for (index = 0; index < 5; index++) {
        lck[index] = lcreate ();
        assert (lck[index] != SYSERR,"Test 5 FAILED creating lock\n");

        ret = lock (lck[index], READ, DEFAULT_LOCK_PRIO);
        assert (ret == OK,"Test 5 FAILED acquiring lock\n");
    }

    ret = releaseall (2, lck[4], lck[0]);
    assert (ret == OK,"Test 5 FAILED releasing 4 & 0\n");

    ret = releaseall (3, lck[1], lck[3], lck[2]);
    assert (ret == OK,"Test 5 FAILED releasing 1 3 & 2\n");
